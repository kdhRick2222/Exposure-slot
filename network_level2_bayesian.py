...
# Amortized(input_units=vis_dim, d_theta=vis_dim // 2, output_units=ctx_dim)
class Amortized(nn.Module):
    def __init__(self, level=2, input_units=128, d_theta=64, output_units=128):
        super(Amortized, self).__init__()
        self.output_units = output_units
        self.level = level

        if self.level == 2:
            self.main_weight_mean = InferenceBlock(input_units, d_theta, output_units*3)
            self.main_weight_log_variance = InferenceBlock(input_units, d_theta, output_units*3)
            self.sub_weight_mean = InferenceBlock(input_units, d_theta, output_units*7)
            self.sub_weight_log_variance = InferenceBlock(input_units, d_theta, output_units*7)
            self.eps1 = nn.Parameter(torch.randn(3, 1, 128))
            self.eps2 = nn.Parameter(torch.randn(7, 1, 128))
        elif self.level == 3:
            self.main_weight_mean = InferenceBlock(input_units, d_theta, output_units*3)
            self.main_weight_log_variance = InferenceBlock(input_units, d_theta, output_units*3)
            self.sub_weight_mean = InferenceBlock(input_units, d_theta, output_units*7)
            self.sub_weight_log_variance = InferenceBlock(input_units, d_theta, output_units*7)
            self.sub2_weight_mean = InferenceBlock(input_units, d_theta, output_units*10)
            self.sub2_weight_log_variance = InferenceBlock(input_units, d_theta, output_units*10)
            self.eps1 = nn.Parameter(torch.randn(3, 1, 128))
            self.eps2 = nn.Parameter(torch.randn(7, 1, 128))
            self.eps3 = nn.Parameter(torch.randn(10, 1, 128))

    def sample(self, mu, logvar, eps):
        B, dim = mu.shape
        prompt_num = dim//128
        mu = mu.reshape(B, prompt_num, 128)
        logvar = logvar.reshape(B, prompt_num, 128)
        eps = torch.randn(mu.shape).type_as(mu)
        bias = mu + eps * logvar.exp().sqrt()
        return bias

    def forward(self, inps):
        if self.level==2:
            main_weight_mean = self.main_weight_mean(inps)
            main_weight_log_variance = self.main_weight_log_variance(inps)
            main_bias = self.sample(main_weight_mean, main_weight_log_variance, self.eps1)
            sub_weight_mean = self.sub_weight_mean(inps)
            sub_weight_log_variance = self.sub_weight_log_variance(inps)
            sub_bias = self.sample(sub_weight_mean, sub_weight_log_variance, self.eps2)
            return [main_weight_mean, main_weight_log_variance, main_bias], [sub_weight_mean, sub_weight_log_variance, sub_bias]
        elif self.level==3:
            main_weight_mean = self.main_weight_mean(inps)
            main_weight_log_variance = self.main_weight_log_variance(inps)
            main_bias = self.sample(main_weight_mean, main_weight_log_variance, self.eps1)
            ...
            sub_weight_log_variance = self.sub_weight_log_variance(inps)
            sub_bias = self.sample(sub_weight_mean, sub_weight_log_variance, self.eps2)
            sub2_weight_mean = self.sub2_weight_mean(inps)
            sub2_weight_log_variance = self.sub2_weight_log_variance(inps)
            sub2_bias = self.sample(sub2_weight_mean, sub2_weight_log_variance, self.eps3)
            return [main_weight_mean, main_weight_log_variance, main_bias], [sub_weight_mean, sub_weight_log_variance, sub_bias], [sub2_weight_mean, sub2_weight_log_variance, sub2_bias]


# Amortized(input_units=vis_dim, d_theta=vis_dim // 2, output_units=ctx_dim)
class Amortized_alpha(nn.Module):
    def __init__(self, level=2, input_units=128, d_theta=64, output_units=128):
        super(Amortized_alpha, self).__init__()
        self.output_units = output_units
        self.level = level
        ...
        if self.level == 2:
            self.main_weight_mean = InferenceBlock(input_units, d_theta, output_units*3)
            self.main_weight_log_variance = InferenceBlock(input_units, d_theta, output_units*3)
            self.main_alpha = InferenceBlock(input_units, d_theta, 3)
            self.sub_weight_mean = InferenceBlock(input_units, d_theta, output_units*7)
            self.sub_weight_log_variance = InferenceBlock(input_units, d_theta, output_units*7)
            self.sub_alpha = InferenceBlock(input_units, d_theta, 7)
            # self.alpha1 = nn.Parameter(torch.randn(3, 1, 1))
            # self.alpha2 = nn.Parameter(torch.randn(7, 1, 1))
        elif self.level == 3:
            self.main_weight_mean = InferenceBlock(input_units, d_theta, output_units*3)
            self.main_weight_log_variance = InferenceBlock(input_units, d_theta, output_units*3)
            self.main_alpha = InferenceBlock(input_units, d_theta, 3)
            ...
            self.sub2_weight_mean = InferenceBlock(input_units, d_theta, output_units*10)
            self.sub2_weight_log_variance = InferenceBlock(input_units, d_theta, output_units*10)
            self.sub2_alpha = InferenceBlock(input_units, d_theta, 10)
            # self.alpha1 = nn.Parameter(torch.randn(3, 1, 1))
            # self.alpha2 = nn.Parameter(torch.randn(7, 1, 1))
            # self.alpha3 = nn.Parameter(torch.randn(10, 1, 1))

    def sample(self, mu, logvar, alpha):
        B, dim = mu.shape
        prompt_num = dim//128
        mu = mu.reshape(B, prompt_num, 128)
        logvar = logvar.reshape(B, prompt_num, 128)
        eps = torch.randn(mu.shape).type_as(mu)
        bias = mu + eps * logvar.exp().sqrt()
        return bias*torch.sigmoid(alpha.unsqueeze(-1))

    def forward(self, inps):
        if self.level==2:
            main_weight_mean = self.main_weight_mean(inps)
            main_weight_log_variance = self.main_weight_log_variance(inps)
            main_alpha = self.main_alpha(inps)
            main_bias = self.sample(main_weight_mean, main_weight_log_variance, main_alpha)
            sub_weight_mean = self.sub_weight_mean(inps)
            sub_weight_log_variance = self.sub_weight_log_variance(inps)
            sub_alpha = self.sub_alpha(inps)
            sub_bias = self.sample(sub_weight_mean, sub_weight_log_variance, sub_alpha)
            return [main_weight_mean, main_weight_log_variance, main_bias], [sub_weight_mean, sub_weight_log_variance, sub_bias]
        elif self.level==3:
            main_weight_mean = self.main_weight_mean(inps)
            main_weight_log_variance = self.main_weight_log_variance(inps)
            ...
            sub2_weight_mean = self.sub2_weight_mean(inps)
            sub2_weight_log_variance = self.sub2_weight_log_variance(inps)
            sub2_alpha = self.sub2_alpha(inps)
            sub2_bias = self.sample(sub2_weight_mean, sub2_weight_log_variance, sub2_alpha)
            return [main_weight_mean, main_weight_log_variance, main_bias], [sub_weight_mean, sub_weight_log_variance, sub_bias], [sub2_weight_mean, sub2_weight_log_variance, sub2_bias]
...
class sub_SlotAttention(nn.Module):
    ...
    def forward(self, inputs, q_inp, k_inp, v_inp, sub_bayesian_list): # [B, HW, C]
        b, n, d, device, dtype = *inputs.shape, inputs.device, inputs.dtype
        n_s = self.num_slots

        slots = self.slots_mu.repeat(b, 1, 1)
        prompts = self.prompts.repeat(b, 1, 1) #+ sub_bayesian_list[-1]
...
class Slot_in_slot_Attention(nn.Module):
    ...
    def forward(self, inputs, q_inp, k_inp, v_inp, main_bayesian_list, sub_bayesian_list): # [B, HW, C]
        b, n, d, device, dtype = *inputs.shape, inputs.device, inputs.dtype
        n_s = self.num_slots

        slots = self.slots_mu.repeat(b, 1, 1)
        prompts = self.prompts.repeat(b, 1, 1) #+ main_bayesian_list[-1]
        k, v = self.norm_k_inp(k_inp), self.norm_v_inp(v_inp)
        ...
        for _ in range(self.iters):
            ...
            sub_slots, sub_prompts, sub_slotfeature, sub_promptfeature, sub_attn = self.sub_slot_attention(inputs * prompt_feature_.permute(0, 2, 1), q, k, v, sub_bayesian_list)
            slots = slots.reshape(b, -1, d)
            slots = slots + self.mlp(self.norm_pre_ff(slots))
...
class IG_MSA(nn.Module):
    ...
    def forward(self, x_in, main_bayesian_list, sub_bayesian_list):
        """
        x_in: [b,h,w,c]
        illu_fea: [b,h,w,c]
        return out: [b,h,w,c]
        ...
        for i in range(len(self.slot_list)):
            ...
            slots_, attn_maps_, prompt_map_, slot_map_, slot_cossim_total = slot_attention(x, q_inp[:, :, i*c:(i+1)*c], k_inp[:, :, i*c:(i+1)*c], v_inp[:, :, i*c:(i+1)*c], main_bayesian_list, sub_bayesian_list) # slot_map.shape == [8, 4096, 128]
...
class IGAB(nn.Module):
    ...
    def forward(self, x, main_bayesian_list, sub_bayesian_list):
        """
        x: [b,c,h,w]
        return out: [b,c,h,w]
        """
        ...
        for (attn, ff) in self.blocks:
            x_attn, attn_maps, slot_features, slots_cossim_list = attn(x, main_bayesian_list, sub_bayesian_list)
...
class Slot_model(nn.Module):
    ...
    def forward(self, x, gt, bayesian_net, inference=False):
        B, C, H, W = x.shape
        dH = H%4
        dW = W%4
        if dH!=0 or dW!=0:
        ...
        # [main_weight_mean, main_weight_log_variance, main_bias], [sub_weight_mean, sub_weight_log_variance, sub_bias]
        main_bayesian_list, sub_bayesian_list = bayesian_net(torch.mean(conv3, dim=(2, 3)))
        feature_i, attn_maps, slot_features, slots_cossim_list = self.TransformerBlock(conv3, main_bayesian_list, sub_bayesian_list)
        ...
        return output, recon_slot, feature_loss, [main_bayesian_list[0], sub_bayesian_list[0]], [main_bayesian_list[1], sub_bayesian_list[1]]
